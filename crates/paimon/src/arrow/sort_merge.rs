...
///
/// For deduplicate: returns the single winner (batch_idx, row_idx).
/// For future merge functions (partial-update, aggregate): may need
/// to materialize a new row.
pub(crate) trait MergeFunction: Send + Sync {
    /// Pick the winning row from same-key candidates.
    /// Returns `(batch_idx, row_idx)` of the winner in the batch buffer.
    fn pick_winner(&self, rows: &[MergeRow]) -> (usize, usize);
}

/// Deduplicate merge: keeps the row with the highest sequence number.
...
impl MergeFunction for DeduplicateMergeFunction {
    fn pick_winner(&self, rows: &[MergeRow]) -> (usize, usize) {
        ...
        (winner.batch_idx, winner.row_idx)
    }
}
...
impl SortMergeCursor {
    ...
    fn sequence_number(&self, seq_index: usize) -> i64 {
        ...
        let arr = col.as_any().downcast_ref::<Int64Array>().expect(
            "_SEQUENCE_NUMBER column must be Int64",
        );
...
pub(crate) struct SortMergeReaderBuilder {
    ...
    /// Full schema of the input streams (key + seq + value columns).
...
fn sort_merge_stream(
    ...
    Ok(try_stream! {
        // Initialize cursors: read first batch from each stream.
        ...
        loop {
            ...
            loop {
                ...
                // Advance the cursor.
                {
                    ...
                    if cursor.is_finished() {
                        // Try to get next batch from this stream.
                        cursors[current_winner] = None;
                        if let Some(batch_result) = streams[current_winner].next().await {
                            let batch = batch_result?;
                            if batch.num_rows() > 0 {
                                let rows = convert_batch_keys(&batch, &key_indices, &mut row_converter)?;
            ...
            let (win_batch_idx, win_row_idx) = merge_function.pick_winner(&same_key_rows);
...
mod tests {
    ...
    async fn test_loser_tree_basic() {
        ...
        let s0 = stream_from_batches(vec![make_batch(&schema, vec![1, 3], vec![1, 1], vec![Some("a"), Some("c")])]);
        let s1 = stream_from_batches(vec![make_batch(&schema, vec![2, 4], vec![1, 1], vec![Some("b"), Some("d")])]);
